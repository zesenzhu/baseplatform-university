...
import {
  introduceChange,
  INTRODUCE_CHANGE_FOR_CANVAS,
} from "../store/introduce";
  ...
  return (
    <div className={"bottom_wrapper"}>
      ...
      {skin === "ai_exam" ? (
        ...
        >
          <div className={"animate-wrapper"}>
            <div className={"swiper-slide ai_exam_slide slider1"}></div>
          </div>

          <div className={"animate-wrapper"}>
            <div className={"swiper-slide ai_exam_slide slider2"}></div>
          </div>
      ...
      {skin === "ai_practice"
        ? introduce[skin].list.map((i, k) => {
            return (
              <div
                key={i.title}
                className={`ai_practice_animation_bg ${
                  introduce[skin].active === k ? "active" : ""
                } pic${k + 1}`}
              ></div>
            );
          })
