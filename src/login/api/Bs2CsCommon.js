...
//记录用户网页登陆时长，网页需每隔一分钟发送一次，UserID为登陆的用户ID，SysID为用户访问的系统；
//url为基础平台地址“http://基础平台IP:基础平台port/虚拟路径（如有）/”，TargetSysID为推送的目标系统，如890或850，一体化智慧校园内应推850
BsToCsFunc.prototype.online = function (proName, moduleName, UserID, SysID, url, TargetSysID) {
    if (this.objState == 0) {
        if (this.objConnect) {
            var char1 = String.fromCharCode(1);
            var char2 = String.fromCharCode(2);
            //给exe发信息
            this._Socket.send(proName + char1 + moduleName + char1 + "003" + char2 + UserID + char2 + SysID + char2 + url + char2 + TargetSysID);
            return true;
        }
    }
    return false;
