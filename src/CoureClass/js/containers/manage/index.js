...
function Index(props) {
  ...
  const {
    ...
    teacherID,
    teacherName,
  } = useSelector((state) => state.breadCrumb.manage);
  ...
  useEffect(() => {
    ...
    if (SchoolID) {
      ...
      if (collegeID && collegeName && gradeID && gradeName) {
        ...
        gradesRef.current = {
          ...gradesRef.current,
          dropSelectd: { value: gradeID, title: gradeName },
        };
      ...
      if (subjectID && courseID) {
        subjectsRef.current = {
          ...subjectsRef.current,
          dropSelectd: { value: subjectID, title: subjectName },
        };

        coursesRef.current = {
          ...coursesRef.current,
          dropSelectd: { value: courseID, title: courseName },
        };
      ...
      const key =
        searchRef.current.CancelBtnShow === "n" ? "" : searchRef.current.value;

      const pageSize = paginationRef.current.pageSize;

      const pageIndex = paginationRef.current.current;

      const GetAllInfo = GetAllTeachInfo_university({
        schoolID: SchoolID,
        dispatch,
      });

      const GetCourseClassInfoForPage = GetCourseClassInfoForPage_University({
        schoolID: SchoolID,
        ...
        if (!isUnmount) {
          ...
          if (res[0]) {
            ...
            if (data.SubjectItem && data.SubjectItem.length > 0) {
              subjectList = data.SubjectItem.map((i) => ({
                value: i.SubjectID,
                title: i.SubjectName,
              }));
            }
            ...
            if (data.CollegeItem && data.CollegeItem.length > 0) {
              collegeList = data.CollegeItem.map((i) => ({
                value: i.CollegeID,
                title: i.CollegeName,
              }));
            }
            ...
            if (data.GradeItem && data.GradeItem.length > 0) {
              gradeList = data.GradeItem.map((i) => ({
                value: i.GradeID,
                title: i.GradeName,
              }));
            }
            ...
            if (parseInt(ProductType) === 6) {
              ...
              if (courseID && subjectID) {
                if (data.CourseItem && data.CourseItem.length > 0) {
                  courseList = data.CourseItem.filter(
                    (i) => i.SubjectID === subjectID
                  ).map((i) => ({ value: i.CourseNO, title: i.CourseName }));
                }
          ...
          if (res[1]) {
            let tableList =
              res[1].Item && res[1].Item.length > 0
                ? res[1].Item.map((i, k) => {
                    const NO = createNO(k);
      ...
      return [
...

        {
          ...
          render: (i, data) => {
            return (
              <Button
                type={"link"}
                onClick={(e) => openClassDetail(data.CourseClassID)}
                ...
                {i.TeacherID ? (
                  <>
                    <i
                      ...
                      style={{ backgroundImage: `url(${i.PhotoPath})` }}
                    ></i>

                    <div className={"teacher-msg"}>
                      <div className={"name"} title={i.TeacherName}>
                        {i.TeacherName}
                      </div>

                      <div className={"id"} title={i.TeacherID}>
                        {i.TeacherID}
                      </div>
                    </div>
                  </>
...

        {
          ...
          render: (i) => {
            ...
            return (
              <>
                <div className={"sub-course"} title={i.CourseName}>
                  {i.CourseName}
                </div>
          ...
          render: (i) => {
            return (
              <div className={"cooperate"}>
                <Button
                  className={"edit btn-blue-default"}
                  onClick={(e) => editCourseClass(i.CourseClassID)}
                >
                  编辑
                </Button>

                <Button
                  className={"del btn-blue-default"}
                  onClick={(e) => delCourseClass(2, i.CourseClassID)}
                >
                  删除
                </Button>
              </div>
            ...
            return (
              <>
                <div
                  ...
                  title={`${i.SubjectName}>${i.CourseName}`}
                >
  ...
  const subjectChange = useCallback((data) => {
    ...
    if (dropsInfo.CourseItem && dropsInfo.CourseItem instanceof Array) {
      courseList = dropsInfo.CourseItem.filter(
        (i) => i.SubjectID === value
      ).map((i) => ({ value: i.CourseNO, title: i.CourseName }));
    }
  ...
  const searchClick = useCallback((data) => {
    ...
    if (result) {
      ...
      dispatch(
        showWarnAlert({ title: "输入的教师姓名或教学班名称格式不正确" })
      );
    }
  }, []);
  ...
  const addEditOk = () => {
    const {
      ...
      showModalLoading,
      hideModalLoading,
    } = AddEditClassRef.current;

    let courseClassOk = false,
      courseOk = false,
      teacherOk = false,
      gradeOk = false;
  ...
  const combineOk = useCallback(() => {
    ...
    if (courseClassOk && teacherOk) {
      ...
      CombineCourseClass_University({
        ...
        UserID,
        UserType,
        CourseClassIDs: CombinedClassIDs,
  ...
  const combineCourseClass = useCallback(() => {
    ...
    if (list.length < 2) {
      ...
      if (checkCourseNOList.length > 1) {
        dispatch(
          showWarnAlert({ title: "选择的班级必须是在同一课程下的班级" })
        );
      } else {
        setCombineModal((d) => ({
          ...d,
          show: true,
          courseNO: checkCourseNOList[0],
        }));
      }
    }
  }, []);
  ...
  return (
    <Loading spinning={loading} tip={"加载中,请稍候..."}>
      <div className={"manage-wrapper"}>
        ...
        <div className={"drops-search-wrapper"}>
          ...
          <DropDown
            ...
            dropSelectd={courses.dropSelectd}
            dropList={courses.dropList}
          ...
          <DropDown
            ...
            dropSelectd={grades.dropSelectd}
            dropList={grades.dropList}
          ></DropDown>

          <span className={"result"}>
            共找到<span className={"red"}>{pagination.total}</span>个教学班
          </span>
            ...
            {pagination.total > 0 ? (
              <CheckBoxGroup onChange={checkChange} value={check.list}>
                <Table
                  pagination={false}
                  columns={columns}
                  dataSource={dataSource.courseClass}
                ></Table>
              </CheckBoxGroup>
            ) : (
        ...
        <div className={"course-class-bottom"}>
          {pagination.total > 0 ? (
            <div className={"selector-all"}>
              <CheckBox checked={check.checked} onClick={checkAll}>
                全选
              </CheckBox>

              <Button type={"primary"} onClick={(e) => delCourseClass(1)}>
                删除
              </Button>
          ...
          <PagiNation
            pageSizeOptions={pagination.pageSizeList}
            hideOnSinglePage={
              pagination.pageSize === parseInt(pagination.pageSizeList[0])
            }
      ...
      >
        {addEditCourse.show ? (
          <AddEditCourseClass
            subjectSelectd={subjects.dropSelectd}
            courseSelectd={courses.dropSelectd}
            IsEdit={addEditCourse.isEdit}
            CourseInfo={addEditCourse.CourseInfo}
            CourseClassID={addEditCourse.courseClassID}
            ref={AddEditClassRef}
          ></AddEditCourseClass>
        ) : null}
      </Modal>
      ...
      >
        {combineModal ? (
          <CombineCourseClass
            checkClassProps={check.list}
            dispatch={dispatch}
            CourseNO={combineModal.courseNO}
            LoginUser={LoginUser}
            ref={CombineCourseRef}
          ></CombineCourseClass>
        ) : (
          ""
        )}
