...
function SchoolSetting(props) {
  ...
  const schoolNameBlur = useCallback(() => {
    ...
    if (schoolName) {
      ...
      setSchoolName((d) => {
        schoolNameRef.current = { ...d, tip: true, title: "请输入院校名称" };

        return { ...d, tip: true, title: "请输入院校名称" };
      });
    }
  }, []);
      ...
      if (result) {
        ...
        setSchoolCode((d) => {
          schoolCodeRef.current = {
            ...d,
            tip: true,
            title: "院校代码有英文字母或数字组成",
          };

          return { ...d, tip: true, title: "院校代码有英文字母或数字组成" };
        });
      }
    } else {
      setSchoolCode((d) => {
        schoolCodeRef.current = { ...d, tip: true, title: "请输入院校代码" };

        return { ...d, tip: true, title: "请输入院校代码" };
      });
    }
  }, []);
    ...
    <Loading spinning={loading} tip={"加载中,请稍候..."}>
      ...
      <div className={"school-setting"}>
        <table className={"school-setting-tb"}>
          <tbody>
            <tr>
              <td className={"col1 school-sign"}>院校标志:</td>
