...
class Subject extends Component {
  ...
  weekDateChange(date, week, weekDay) {
    ...
    dispatch({
      type: STSAction.MANAGER_STS_NOW_WEEK_DAY_CHANGE,
      data: weekDay,
    });
  ...
  clickRow(record) {
    ...
    dispatch({
      type: STSAction.SUBJECT_TEACHER_SCHEDULE_UPDATE,
      data: schedule,
    });
  }
  ...
  ScheduleDetailShow(Params) {
    ...
    const {
      ...
      ItemClassHourCount,
      NowClassHourNO,
    } = Manager.SubjectCourseGradeClassRoom;

    const WeekNO = Manager.SubjectTeacherSchedule.NowWeekNO;

    dispatch({
      type: SDActions.COMPONENT_SCHEDULE_DETAIL_MODAL_PARAMS_UPDATE,
      data: {
        ItemClassHour,
        ItemClassHourCount,
        NowClassHourNO,
  ...
  render() {
    ...
    //封装获取到的学科列表
    if (
      SubjectCourseGradeClassRoom.ItemSubject &&
      SubjectCourseGradeClassRoom.ItemSubject.length > 0
    ) {
    ...
    return (
      ...
      >
        <div
          className={`subject-teacher-subject-content ${
            this.state.fullScreen ? "full-screen-doing" : ""
          }`}
        >
          {/*<div className="full-screen-btn" onClick={this.FullScreenClick.bind(this)}>{this.state.fullScreen?'退出全屏':'全屏'}</div>*/}

          <Loading
            spinning={SubjectTeacherSchedule.loadingShow}
            tip="正在为您查找，请稍后..."
          >
            <DropDown
