...
class AddAppilicationModal extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      classResultImgUrl: props.AccessData.ImgUrlProxy+props.AccessData.AddAppilicationParams.ApplicationImgUrl,
      picVisible: false,
    };
  }
  // 名称change
  ...
  handleGetResultImgUrl = (key) => (blob, filePath) => {
    const { dispatch, AccessData:{ImgUrlProxy} } = this.props;
  ...
  render() {
    ...
    console.log(this.state.classResultImgUrl)
    return (
      <div className="AddAppilicationModal">
        <div className="access-main-msg-box">
          ...
          <span className="row clearfix rigth-row">
            <span className="access-main-msg-tips">
              <span
                style={{
                  display:
                    this.props.type !== "add" && IsThirdParty === "0"
                      ? "none"
                      : "inline",
                }}
                className="must-icon"
          ...
          <span className="row clearfix left-row add-row">
            <span className="access-main-msg-tips">应用访问入口：</span>
            <span
              title={"当前已添加" + Entrances.length + "个入口"}
              className="access-main-msg"
            >
              当前已添加<span style={{ color: "red" }}>{Entrances.length}</span>
              个入口
              <span
                onClick={this.onAddAddrClick.bind(this, ApplicationID)}
                className="add-addr"
              >
                +添加访问入口
              </span>
        ...
        {Entrances instanceof Array && Entrances.length > 0 ? (
              ...
              <thead className="ant-table-thead table-top">
                <tr>
                  ...
                  >
                    <span className="ant-table-header-column">
                      <div>
                        <span className="ant-table-column-title">操作</span>
                        <span className="ant-table-column-sorter"></span>
                      </div>
                    </span>
                  </th>
